...
from rolepermissions.verifications import has_role, has_roles, has_permission, has_object_permission
...
class HasRoleTests(TestCase):
    ...
    def test_user_has_VerRole1_and_VerRole2(self):
        user = self.user

        VerRole2.assign_role_to_user(user)

        self.assertTrue(has_roles(user, [VerRole1, VerRole2]))
    ...
    def test_user_has_VerRole1_and_VerRole3_by_name(self):
        user = self.user

        VerRole3.assign_role_to_user(user)

        self.assertTrue(has_role(user, ['ver_role1', 'ver_role3']))
...
class HasObjectPermissionTests(TestCase):
    ...
    def setUp(self):
        ...
        def obj_checker(roles, user, obj):
            return obj and True
